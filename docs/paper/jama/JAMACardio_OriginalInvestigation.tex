...
the finding is a measured null and not an underpowered one.
...
the same meshes at negligible cost. Three conditions decide whether a spectral
index earns a place beside them. It has to improve discrimination when added to
those features. The improvement must be large enough to matter, judged against a
threshold fixed in advance and not against a $p$ value alone. And it must
survive cross-validation that holds out whole patients and shape families, so
that anatomy cannot leak between training and test folds. Evaluations meeting
all three conditions have not been reported.
...
part of the null and not all of it. Where the competitors already carry the
information, there is nothing left for \SFI{} to add; where unique variance
...
reported as confounding and not as a win for \SFI{}. Under logistic
...
$0.250$ is not weak; it is anti-predictive, and $0.447$ remains below chance.
Three checks agree. All $15$ features, competitor and \SFI{} alike, were
univariately at chance ($0.435$--$0.547$). The gradient-boosting arm, whose
baseline sat at $0.516$, gave $\dAUC=+0.021$ ($p=0.738$). And replacing the nine
\SFI{} columns with nine columns of standard Gaussian noise, then re-running the
identical pipeline, reproduced the increment: the noise gained $+0.137$ on
...
the baseline to $0.345$. That primary endpoint was \textbf{not met}, the
