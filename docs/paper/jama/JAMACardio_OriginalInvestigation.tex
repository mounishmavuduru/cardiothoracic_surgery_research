...
the pre-registered $0.05$ threshold; the primary endpoint was not met. The
clinical endpoint in $82$ patients was also not met.

\paragraph{Meaning.} The index added nothing beyond established fibrosis and
connectivity features. Because the interval excludes the pre-specified effect
size, this is an informative null rather than an underpowered study.
...
used. The Roney virtual cohort~\cite{roney2022} (Zenodo 5801337) contains 100 meshes and all 100
...
pooled and compared by the paired DeLong test~\cite{delong1988} in its fast midrank form~\cite{sunxu2014}; because
...
for a cohort of atrial fibrillation patients and is flagged as anomalous~\cite{kumar2012,oral2008,marquardt2018,kawai2019,liu2020}. Up to
...
2-manifold exponent~\cite{weyl1912} of $-1.0$, so meshing the same anatomy more finely degrades
...
as containing zero (Table~1, Figure~1). On $182$ patient-derived anatomies this
is a measured null rather than a shortfall of power. It does not exclude a small
effect: the lower bound of $-0.017$ leaves an increment of a few thousandths
compatible with these data, resolvable only in a far larger cohort. The
synthetic arm behaved the same way, the
effect converging to $\dAUC\approx+0.003$ across up to $100{,}000$ networks.

The null is not a computation failure. The exact per-region $\Delta\lam_2$ SFI,
which carries no first-order approximation error, also added nothing over the
competitor set. Measuring the mechanism directly and without reference to the
...
estimator predictably rather than improving it. The screen is portable to
spectral fragility markers used outside cardiology, but it bounds estimator
accuracy only; it does not establish that a feature ranks cases poorly, which
must still be tested against competitors.
...
were joined, identified this as an artefact of baseline degradation, the
baseline being anti-predictive at AUC $0.250$ and reaching only $0.447$ with SFI
added, and nine columns of Gaussian noise reproduced the increment, gaining
$+0.137$ on average with $6$ of $20$ draws matching or exceeding SFI (empirical
$p=0.333$; Table~2). The fixed-sequence stopping rule then halted testing. The
guard that caught the false positive was written before the outcome column was
opened, not after the result was seen.
...
window, the inducibility rate falling from $58\%$ at $1500$ nodes to $17\%$ at
$3000$ and rising to $37.5\%$ at $\sim\!50$k, against $34.0\%$ at the
$2{,}000$-node operating resolution, with only $7/24$ subjects consistent across
all six tiers. Per-subject label noise is non-differential and biases every
reported increment toward zero, so the null is partly a statement about the
labeller. The clinical arm is underpowered by construction: with $48/82$ events,
...
clinical endpoint was not met and could not have resolved an effect of the size
that would matter. What is offered is a bounded negative and a portable,
label-free validity screen.
