...
the released UW meshes carry a constant fibre vector, which removes the fibre
heterogeneity that seeds unidirectional block, and the in-silico inducer is
under-aggressive. This is a base-rate
...
\item The UW cohort uses a PCA-surrogate universal-atrial-coordinate map, and the
released meshes carry a \emph{degenerate} fibre field: the \code{VECTORS fiber}
array is present but equals a constant $(1,0,0)$ for every element of all $164$
meshes, so the anisotropy our solver applies is a fixed coordinate bias rather
than anatomy. Since fibre heterogeneity is a principal substrate for
unidirectional block, this is the most likely reason the cohort is only
$\sim\!7\%$ inducible, at or below the no-AF-control clinical floor. Its
predictive test is therefore uninformative about SFI.
