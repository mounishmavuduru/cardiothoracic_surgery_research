...
% lightweight, compile-safe "scope" callout
\newenvironment{scopebox}
  {\par\smallskip\noindent\begin{tabular}{@{\hspace{2pt}}!{\color{gray!60}\vrule width 2pt}@{\hspace{8pt}}p{0.955\linewidth}@{}}}
...
\author{[Author name]\\ \small [Affiliation / school]\thanks{Independent
in-silico computational-cardiology project. Author and affiliation are
placeholders to be completed before submission. Correspondence and full
source: see the project repository.}}
...
\begin{abstract}
\noindent
We ask whether a closed-form spectral graph biomarker --- the \textbf{Spectral
Fragility Index (SFI)}, the Fiedler-value sensitivity
$\partial\lam_2/\partial w_{ij}=(\phii_i-\phii_j)^2$ aggregated under a
fibrosis-weighted conduction-uncoupling field --- predicts reentry
inducibility in atrial tissue beyond established fibrosis and connectivity
features. Pre-registered before any labels were seen, the primary endpoint was
$\dAUC\geq 0.05$ with a DeLong paired $p<0.05$ on shape-family-held-out folds.
\emph{The endpoint is not met --- but the strength of the negative differs by
regime, and we separate them.} On up to $100{,}000$ synthetic excitable
networks SFI adds no practically meaningful predictive value (subspace
$\dAUC\approx+0.003$; $\sim\!4{,}150$ cases needed at $80\%$ power --- real only
because $N$ is huge, hence \emph{clinically undetectable} there). On the two
real cohorts (Roney $n=62$, UW/Boyle $n=82$) the result is
\textbf{underpowered and inconclusive, not a clean null}: the combined
gradient-boosting point estimate is $+0.051$ (which \emph{exceeds} the $0.05$
threshold) but fails paired significance (DeLong $p=0.155$), so real anatomy
cannot \emph{exclude} a moderate effect. Where the null \emph{is} clean (the
synthetic cohort), the mechanism is \textbf{feature redundancy}: the exact
per-region $\Delta\lam_2$ SFI --- which has no first-order approximation error
--- also adds nothing over the competitor set. A separate, label-free result
explains why the \emph{single-vector} SFI is additionally ill-conditioned on
real tissue: it is valid only while $\rhoval=\lVert\Delta L\rVert/(\lam_3-\lam_2)=O(1)$,
and real atria have $\rhoval\approx 2422$ ($\sim\!10^3\times$ past the empirical
$\rhoval^\ast\approx 3$), so by Davis--Kahan the Fiedler vector is unstable. We
do \textbf{not} claim $\rhoval$ proves non-predictiveness --- it bounds the
...
imaging, by a mechanism we make precise. Finally, naive methodology (random CV,
$p$-value thresholds, no effect-size gate) would have reported SFI as working;
our pre-registered protocol rejects it. A key limitation surfaced by our own
convergence study: the inducibility labeller over-calls at the operating mesh
resolution and, on a completed six-tier study ($1500$--$48000$ nodes, $n=24$),
does not converge within the tested window. The contribution is a
rigorously-scoped negative with its mechanism, a label-free estimator-validity
criterion, a scaling characterization, and a worked demonstration of the false
positives standard methodology produces.
\end{abstract}
...
fragility. We test this hypothesis and, crucially, \emph{pre-register the null}
...
The mathematics we use is classical; we claim no new theorem. What is ours is
...
\begin{scopebox}
\textbf{What this paper claims, in one paragraph.} A closed-form spectral
...
($\rhoval\gg 1$). The honest core is not a discovery but a carefully-scoped
boundary: where a Fiedler-based estimator is numerically valid, why real tissue
lies far outside it, and how easily naive methodology mistakes redundant,
...
\label{sec:theory}
%======================================================================
The load-bearing mathematics is classical numerical linear algebra; the
...
\begin{scopebox}
\textbf{The identity is verified, not asserted (seven independent ways).}
...
hand-rolled inverse iteration (spread $2.5\times10^{-15}$). Finally, the
algebraic core is \textbf{kernel-checked in the Coq proof assistant} over the
...
($s_{ij}=e_i-e_j$) is well-defined. For genuinely discrete edits (Maze lesion,
atriotomy) the expansion is abandoned and $\lam_2$ is recomputed \emph{exactly}
after the rank-one edit.

\subsection{The validity radius (the honest core)}
\label{sec:validity}
...
\item \textbf{Weyl's inequality} bounds the \emph{eigenvalue} move:
      $\lvert\Delta\lam_2\rvert\leq\lVert\Delta L\rVert$.
\item \textbf{Davis--Kahan / Bauer--Fike}~\cite{daviskahan1970} bound the
...
throughout. \textbf{On real excitable media the gap is tiny, so $\rhoval$ is
enormous: atrial-cohort median $\rhoval\approx 2422$}, about $10^3\times$ past
$\rhoval^\ast$.

\begin{scopebox}
\textbf{What $\rhoval$ proves, and what it does not.} It proves the
...
\textbf{not} prove SFI is non-predictive: $\rhoval$ bounds the accuracy of the
...
\paragraph{Labels.} Ground truth is a genuine monodomain Mitchell--Schaeffer
...
\textbf{The negative is not uniform, and we separate the regimes.} On the
...
null ($6$ positives, competitor base AUC $\approx$ chance), so it functions as an
underpowered external check of the \emph{labeller}, not of SFI.

\textbf{Where the clean null comes from --- feature redundancy, not the validity
radius.} The \emph{exact} per-region $\Delta\lam_2$ SFI, which carries no
first-order approximation error and is immune to the $\rhoval$ argument, also
adds nothing over the competitor set. So the mechanism of the null is that SFI's
information is already present in standard connectivity features
($\lam_2$-alone, min-cut, percolation) --- a label-dependent finding, separate
from $\rhoval$.

\textbf{Secondary endpoint (pre-specified), with multiple-comparisons
disclosure.} SFI beats a \emph{fibrosis-heterogeneity-only} baseline (combined
GBT $\dAUC+0.103$, $p=0.009$): it is \emph{not merely re-encoded fibrosis} and
carries connectivity information beyond the substrate. This is one of $\sim\!30$
DeLong comparisons; treating \code{fibrosis\_vs\_+SFI} as a single pre-specified
secondary endpoint, its $p=0.009$ against a family-wise threshold
$\approx 0.05/12$--$0.05/30$ is \emph{suggestive, not confirmatory}.

\textbf{Power analysis.} In the synthetic cohort the subspace effect ($+0.0028$)
needs $\sim\!4{,}150$ cases for $80\%$ power and the single-vector effect
($+0.0007$) $\sim\!23{,}000$ --- clinically undetectable \emph{there}. We do
\textbf{not} transport ``clinically undetectable'' to the real cohorts, whose
...
(\code{results/\allowbreak gm4\_*\_metrics.json}). Lower rank is better; a perfect localizer
...
\emph{and} oscillator dynamics. Three honesties apply: (i)~the strict endpoint is
...
by \textbf{$+0.117$} (naive random-CV $0.756$ vs.\ grouped $0.640$ on a
controlled replicated cohort); the effect-size gate catches the \textbf{$p$-value
...
--- correctly flagged as anomalous and attributable to the surrogate substrate and
an under-aggressive in-silico inducer, not biology. This is a base-rate
...
again to \textbf{$37.5\%$ at the finest affordable tier ($\sim\!50$k nodes)} --- a
...
\begin{scopebox}
\textbf{What convergence does and does not undermine.} The rate does \emph{not}
...
\section{Discussion}
%======================================================================
\paragraph{What is genuinely contributed.}
...
nothing), and an honest inconclusive/underpowered verdict on real anatomy.
...
with the honest scope that $\rhoval$ bounds estimator accuracy, not predictive
...
\paragraph{Generalization beyond cardiology.} The validity screen is
...
\paragraph{What is NOT claimed.} The mathematics is classical; no new theorem.
...
\item \textbf{The labeller over-calls inducibility at the operating mesh
...
\item The UW cohort uses a PCA-surrogate universal-atrial-coordinate map and
rule-based fibres, and is only $\sim\!7\%$ inducible (at/below the no-AF-control
clinical floor), so its predictive test is uninformative.
...
estimator is provably ill-conditioned on real tissue ($\rhoval\gg1$). The honest
core is not a discovery but a carefully-scoped boundary --- where a Fiedler-based
...
context''} for every theorem: \textbf{zero axioms}. A mathematically identical
...
seizure detection using a Fiedler-value network measure. \emph{Scientific
Reports}, 10:16401, 2020.
...
\bibitem{roney2022} C.~H.~Roney et al. Constructing a human atrial fibre atlas
and a virtual cohort of left-atrial models. \emph{Circulation: Arrhythmia and
Electrophysiology}, 15, 2022. doi:10.1161/CIRCEP.121.010253. Data: Zenodo 5801337.

\bibitem{uwboyle2025} S.~Bifulco, P.~M.~Boyle, et al. Patient-specific atrial
meshes with fibrosis and fibre fields. \emph{Communications Medicine}, 2025.
...
ratio, a normalized measure to enable inter-patient comparability of left atrial
fibrosis. \emph{Heart Rhythm}, 11(1):85--92, 2014.

\bibitem{kumar2012} S.~Kumar et al. Prospective characterization of catheter
ablation-induced atrial fibrillation inducibility. \emph{Circulation: Arrhythmia
and Electrophysiology}, 5(3):531--538, 2012. PMID 22528047.

\bibitem{marquardt2018} J.~Marquardt et al. Inducibility of atrial fibrillation
in patients without a history of AF. \emph{Journal of Atrial Fibrillation},
11(1):1837, 2018. PMID 30455834.

\bibitem{oral2008} H.~Oral et al. Inducibility of atrial fibrillation. \emph{Journal
of Cardiovascular Electrophysiology}, 19(5):466--470, 2008. PMID 18266669.

\bibitem{kawai2019} S.~Kawai et al. Inducibility of atrial fibrillation in
persistent AF. \emph{Journal of Arrhythmia}, 35(2):223--230, 2019. PMID 31007786.

\bibitem{liu2020} X.~Liu et al. Post-ablation atrial fibrillation inducibility
and outcomes. \emph{Journal of the American Heart Association}, 9(14):e015260,
2020. PMID 32654581.
\end{thebibliography}
